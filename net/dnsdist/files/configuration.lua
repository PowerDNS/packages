...
function M.apply(config)
  ...
  local localDomainsDests = newNMG()
  local allAddresses = {}
        ...
        if conf['enabled'] == '1' then
          ...
          for _, addr in ipairs(addresses) do
            ...
            if conf['local_resolution'] == '1' then
              localDomainsDests:addMask(addr)
            end
          ...
          if interfaceDests:size() > 0 then
            ...
            -- sentinel domains
            if conf['sentinel_domains'] ~= nil and interfaceDests:size() > 0 then
              local sentinelItfs = sentinelRules[conf['sentinel_domains']]
              for sentinelItf, rule in pairs(sentinelItfs) do
                ...
                if #addressesList > 0 then
                  addAction(AndRule{itfNMGRule, rule}, SpoofAction(addressesList, { ttl=config['sentinel_domains_ttl'] }))
                end
              end
            end
            ...
            if conf['advertise'] == '1' then
              ...
              if #svc > 0 then
                ...
                addAction(AndRule{QTypeRule(DNSQType.SVCB), itfNMGRule, nameRule}, SpoofSVCAction(svc))
                if config['advertise_for_domain_name'] ~= nil and #config['advertise_for_domain_name'] > 0 then
                  ...
                  addAction(AndRule{itfNMGRule, QNameRule(config['advertise_for_domain_name'])}, SpoofAction(itfAddrs, { ttl=config['sentinel_domains_ttl'] }))
                end
                ...
                addAction(AndRule{itfNMGRule, QNameRule('_dns.resolver.arpa.')}, NegativeAndSOAAction(false, '_dns.resolver.arpa.', 3600, 'fake.resolver.arpa.', 'fake.resolver.arpa.', 1, 1800, 900, 604800, 86400))
              end
            end
          end
        end
